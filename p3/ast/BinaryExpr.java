...
public class BinaryExpr extends Expr {
    ...
    public void check(ArrayList<Map<String, Type>> tables) {
        ...
        if (expr1.type != expr2.type) {
            ...
            Interpreter.fatalError("Type error : " + expr1.type + " " + opStr + " " + expr2.type, Interpreter.EXIT_STATIC_CHECKING_ERROR);
        }
            ...
            case DIV:
                if (isLong) {
                    if ((Long) expr2.value == 0L) {
                        Interpreter.fatalError("Divide 0 error :", Interpreter.EXIT_DIV_BY_ZERO_ERROR);
                    }
                    value = (Long) expr1.value / (Long) expr2.value;
                } else if (isDouble) {
                    if ((Double) expr2.value == 0.0) {
                        Interpreter.fatalError("Divide by 0 error:", Interpreter.EXIT_DIV_BY_ZERO_ERROR);
                    }
                    value = (Double) expr1.value / (Double) expr2.value;
                }
                break;
